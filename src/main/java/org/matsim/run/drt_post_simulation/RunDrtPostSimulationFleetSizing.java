...
public class RunDrtPostSimulationFleetSizing implements MATSimAppCommand {
    @CommandLine.Option(names = "--config", description = "path to config file", required = true)
    private Path configPath;

	@CommandLine.Option(names = "--drt-plans", description = "path to extracted DRT trips file (plans)", required = true)
    ...
    @CommandLine.Option(names = "--step-size", description = "number of vehicles increased for each step (phase 1)", defaultValue = "5")
    private int stepSizeStageOne;

    @CommandLine.Option(names = "--step-size-2", description = "number of vehicles increased for each step (phase 2)", defaultValue = "1")
    ...
    public static void main(String[] args) {
        new RunDrtPostSimulationFleetSizing().execute(args);
    }
    ...
    private String runSimulation(String temporaryConfig) {
        ...
        drtConfigGroup.vehiclesFile = "drt-vehicles-whole-city/" + fleetSize + "-" + seats + "_seater-drt-vehicles.xml";
		...
		PrepareNetwork.prepareDRT(controler.getScenario().getNetwork(), new ShpOptions(drtArea, null, null));
