 ...
 * For the simulation of policy cases input parameters from {@link NetworkOptions} are needed
 */
@CommandLine.Command(header = ":: Open Leipzig Scenario ::", version = RunLeipzigScenariowithAccessibility.VERSION)
...
public class RunLeipzigScenariowithAccessibility extends MATSimApplication {
	...
	private static final Logger log = LogManager.getLogger(RunLeipzigScenariowithAccessibility.class);
	@CommandLine.Mixin
	...
	public RunLeipzigScenariowithAccessibility(@Nullable Config config) {
		super(config);
	}

	public RunLeipzigScenariowithAccessibility() {
		super(String.format("input/v%s/accessibility-leipzig-v%s-10pct.config.xml", VERSION, VERSION));
	}

	public static void main(String[] args) {
		MATSimApplication.run(RunLeipzigScenariowithAccessibility.class, args);
	...
	protected Config prepareConfig(Config config) {
		...
		if (accessibility){
			...
			routeConfigGroup.setRoutingRandomness(0.0);
		}
	...
	protected void prepareControler(Controler controler) {
		...
		if (accessibility){
			...
			AccessibilityConfigGroup accConfig = ConfigUtils.addOrGetModule(scenario.getConfig(), AccessibilityConfigGroup.class);
			accConfig.setComputingAccessibilityForMode(Modes4Accessibility.freespeed, true); // Example for freespeed mode, configure as needed
