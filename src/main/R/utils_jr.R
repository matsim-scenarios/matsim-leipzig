...
join_base_and_policy <- function(base_persons, policy_persons, shp = NULL, crs = "EPSG:25832"){
  # merge 2 data sets; filter to only include people starting at home; 
  # filter people in shapefile
  joined <- base_persons %>%
    left_join(policy_persons %>% dplyr::select(person,executed_score), by = "person") %>%
  ...
  if(is.null(shp)){
    joined <- joined %>% st_intersection(shp %>% st_transform(crs))
  }
